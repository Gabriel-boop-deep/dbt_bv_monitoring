...
with
    stg_propostas_credito as (select * from {{ ref("stg_propostas_credito") }}),

    stg_clientes as (select * from {{ ref("stg_clientes") }}),
    ...
    joined_table as (
        ...
        left join
            stg_clientes on stg_propostas_credito.cliente_id = stg_clientes.cliente_id
    )
